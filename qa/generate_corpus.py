...
# Number-heavy sentences: time, date, decimals+units, codes with internal
# hyphens, ranges, phone numbers, grouped large numbers. Used both for OCR cases
# (does Azure read the digits right?) and TTS samples (do the voices speak them
# naturally, or do numbers "stand out"?). Reported user pain: ka, ru, uk.
NUMBER_TEXTS = {
    "en": "The meeting is set for 14:30 on 2 May 2025. The floor area is 106.4 m², "
          "volume 408.0 m³. Standard SN 2.02.05-2020, class K-5. Phone +1 202 555-0143. "
          "Price 1 250 000 dollars.",
    "ru": "Встреча назначена на 14:30, 2 мая 2025 года. Площадь дома 106.4 м², "
          "объём 408.0 м³. Норматив СН 2.02.05-2020, класс К-5. Телефон +7 999 123-45-67. "
          "Цена 1 250 000 рублей.",
    "uk": "Зустріч призначено на 14:30, 2 травня 2025 року. Площа будинку 106.4 м², "
          "обсяг 408.0 м³. Норматив СН 2.02.05-2020, клас К-5. Телефон +380 67 123-45-67. "
          "Ціна 1 250 000 гривень.",
    "ka": "შეხვედრა დანიშნულია 14:30 საათზე, 2025 წლის 2 მაისს. ფართობი 106.4 მ², "
          "მოცულობა 408.0 მ³. ნორმატივი 2.02.05-2020, კლასი 5. ტელეფონი +995 599 123-456. "
          "ფასი 1 250 000 ლარი.",
}
...
# Number-heavy cases (clean + one mild-degraded) for the Azure-OCR languages.
    ...
    for lang in ["en", "ru", "uk"]:
        ...
        write_case(cases, f"gen-num-{lang}-clean", render(text, font, lang), text, lang,
                   "numbers: time/date/decimals/codes/ranges/phone")
        img, tags = degrade(render(text, get_font(font_paths(lang)[0], 36), lang), rnd, "mild")
        write_case(cases, f"gen-num-{lang}-degraded", img, text, lang,
                   "numbers, degraded: " + "+".join(tags))
